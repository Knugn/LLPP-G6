...
void Ped::Model::updateHeatmapSeq()
{
	...
	//GetSystemTime(&start);
	double start, stop;
	start = TimingUtils::get_wall_time();
...
#define WEIGHTSUM 273
	...
	//GetSystemTime(&stop);
	stop = TimingUtils::get_wall_time();

	totalUpdateTime += stop - start;
	nUpdates++;
}
...
void Ped::Model::printHeatmapTimingsSeq() {
	std::cout << "Average heatmap update time (wall clock): " << (int)(totalUpdateTime * 1000000 / nUpdates) << " us" << std::endl;
}
