...
vector<Ped::Tagent*> ParseScenario::getAgents() const
{
  ...
  std::copy(agents.begin(), agents.end(), std::inserter(agentsWithUniquePosition, agentsWithUniquePosition.begin()));

  return std::vector<Ped::Tagent*>(agentsWithUniquePosition.begin(), agentsWithUniquePosition.end());
}
